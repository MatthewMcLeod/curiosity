...
export GVFParamFuncs, AbstractGVF, GVF, Horde
...
merge(h1::Horde, h2::Horde) =
    Horde([deepcopy(h1.gvfs); deepcopy(h2.gvfs)])

function Base.get(gvfh::Horde; kwargs...)
    C = map(gvf -> get(cumulant(gvf); kwargs...), gvfh.gvfs)
    Γ = map(gvf -> get(discount(gvf); kwargs...), gvfh.gvfs)
    Π_probs = map(gvf -> get(policy(gvf); kwargs...), gvfh.gvfs)
    return C, Γ, Π_probs
end
...
Base.get(gvfh::Horde, state_tp1, preds_tp1) =
    get(gvfh::Horde; state_t = nothing,
    action_t = nothing,
    state_tp1 = state_tp1,
    action_tp1 = nothing,
    preds_tp1)
