...
function get_demon_parameters(lu::InterestTB, learner, demons, obs, state, action, next_obs, next_state, next_action, env_reward)
...
function update!(lu::InterestTB,
                 ...
                 is_terminal,
                 behaviour_pi_func,
                 env_reward) where {M<:AbstractMatrix, LU<:InterestTB}
...
function zero_eligibility_traces!(lu::InterestTB)
    for (k, v) ∈ lu.e
        if eltype(v) <: Integer
            lu.e[k] = Int[]
