...
Base.@kwdef mutable struct StateETB{O,T <: AbstractTraceUpdate} <: LearningUpdate
    lambda::Float64
    opt::O
    hdpi::HordeDPI
...
function get_demon_parameters(lu::StateETB, learner, demons, obs, state, action, next_obs, next_state, next_action, env_reward)
...
function update!(lu::StateETB,
                 ...
                 is_terminal,
                 behaviour_pi_func,
                 env_reward) where {M <: AbstractMatrix,LU <: StateETB}
...
function zero_eligibility_traces!(lu::StateETB)
    for (k, v) ∈ lu.e
        if eltype(v) <: Integer
            lu.e[k] = Int[]
